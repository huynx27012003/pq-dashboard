...
def format_list_registers(regs: List[EDMIRegister]) -> Dict:
    out = []

    valid_values = EDMI_ERROR_CODE._value2member_map_

    for reg in regs:
        err = reg.ErrorCode

        if isinstance(err, EDMI_ERROR_CODE):
            code_val = err.value
            code_name = err.name
        elif err in valid_values:
            code_val = err
            code_name = valid_values[err].name
        else:
            code_val = err
            code_name = "Unknown"
        ...
        out.append({
            "Name": reg.Name,
            "Value": reg.Value,
            "Error": f"0x{code_val:02X} ({code_name})"
        })
...
def format_parsed_profile_data(
    ...
    time_key: str = "time_stamp",
) -> list[dict[str, Any]]:
    channels_count = profile_spec.ChannelsCount
    if channels_count <= 0:
        return []

    max_records = len(fields) // channels_count
    records_count = profile_spec.RecordsCount or max_records
    if records_count > max_records:
        records_count = max_records
    ...
    for record_idx in range(records_count):
        ...
        if start_dt is None:
            ...
            item[time_key] = (start_dt + timedelta(seconds=interval_sec * record_idx)).isoformat()
        else:
            item[time_key] = start_dt.isoformat()
        ...
        for ch in range(channels_count):
            ...
            if vtype is None:
                value = raw
            # elif vtype == EDMI_TYPE.DATE_TIME or vtype == EDMI_TYPE.DATE or vtype == EDMI_TYPE.TIME:
            #     continue
            else:
