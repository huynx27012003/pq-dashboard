...
var dateRangeOptions = {
    ...
    // The dashboard uses an inner scroll container. Attach the picker to it so positioning
    // stays correct when the user scrolls inside the page (not the window).
    "parentEl": ".app-main",
    // Date control sits near the right edge; opening left avoids off-screen overflow.
    "opens": "left",
    "minDate": "01/01/1990",
    "maxDate": "12/31/2030",
    "ranges": {
...
function selectmapgrid(thecontrol) {
    ...
    var mode = ($(thecontrol).val() || '').toString();
    if (mode !== 'Map' && mode !== 'Grid') mode = 'Map';

    // Keep all map/grid selects in sync (existing behavior).
    $('.mapGrid').val(mode);

    // Apply the chosen layout consistently so we never end up with both visible.
    var tabs = ['Events', 'Disturbances', 'Faults', 'Breakers', 'Extensions', 'Trending', 'TrendingData', 'Completeness', 'Correctness'];
    for (var i = 0; i < tabs.length; i++) {
        setMapGridForTab(tabs[i], mode);
    }

    // Do the heavier work only for the active tab.
    if (mode === 'Grid') {
...
// Show either map or grid for a given tab, never both.
function setMapGridForTab(tabName, mode) {
    var $map = $("#theMap" + tabName);
    var $matrix = $("#theMatrix" + tabName);

    if ($map.length === 0 || $matrix.length === 0)
        return;
...
function buildBarChart(data, thediv, siteID, thedatefrom, thedateto) {
    ...
    // Increase right margin so long legend labels don't get clipped.
    var margin = { top: 20, right: 240, bottom: 20, left: 60 },
    ...
    function buildLegend() {
        ////Legend attributes
        var legendX = width + margin.left + 12;
        var legendY0 = margin.top + 10;
        ...
        legend = svg.selectAll(".legend")
            ...
            .attr("id", "chartLegend")
            .attr("class", "legend")
            // Place legend inside the reserved right margin so labels don't get clipped.
            .attr("transform", function (d, i) { return "translate(" + legendX + "," + (legendY0 + i * 20) + ")"; });
...
function populateGridSparklineDataQuality(data, siteID, siteName, makespark, colors) {
    ...
    var qualitySparkId = "sparkbox_" + siteID + "_box_" + currentTab;
    var qualityBoxW = $("#" + qualitySparkId).width() || 0;
    var qualityBarW = Math.max(10, Math.floor((qualityBoxW - 20) / Math.max(sparkvalues.length, 1)) - 2);

    $("#" + qualitySparkId).sparkline(sparkvalues, {
        type: 'bar',
        height: 120,
        barWidth: qualityBarW,
        barSpacing: 2,
...
function populateGridSparklines(data, siteID, siteName, colors) {
    ...
    var sparkId = "sparkbox_" + siteID + "_box_" + currentTab;
    var sparkBoxW = $("#" + sparkId).width() || 0;
    var barW = Math.max(4, Math.floor((sparkBoxW - 20) / Math.max(numbers.length, 1)) - 2);

    $("#" + sparkId).sparkline(numbers, {
        type: 'bar',
        height: 120,
        barWidth: barW,
        barSpacing: 2,
...
function resizeMapAndMatrix(newTab) {
    ...
    // Keep docklet heights stable even when one portlet is collapsed.
    // (Otherwise the remaining visible docklet "bloats" to fill the space.)
    // Also: avoid ultra-short charts on smaller viewports by enforcing a minimum height.
    var $col = $("#Portlet1" + newTab).closest(".column");
    var totalPortletsInColumn = $col.children(".portlet").length;
    if (totalPortletsInColumn > 0) {
        var chartheight = (columnheight - 24) / totalPortletsInColumn;
        chartheight = Math.max(420, chartheight);
...
function resizeMatrixCells(newTab) {
    ...
    if (h > 0 && w > 0 && r > 0) {
        // Prefer larger, readable square tiles. If there are many tiles, allow scroll instead of tiny blurry cells.
        // Target 4 tiles per row on desktop so the grid is comparable at a glance.
        // On smaller widths, let columns drop automatically.
        var force4Cols = (w >= 1100 && r >= 4);
        var minCellSize = 240;
        var gap = 10; // Should match CSS margins/padding closely.

        var columns = force4Cols ? 4 : Math.max(1, Math.floor((w - gap) / (minCellSize + gap)));
        var rows = Math.ceil(r / columns);

        var cellW = Math.floor((w - (columns + 1) * gap) / columns);

        // Don't go below minimums; instead rely on scroll.
        cellW = Math.max(minCellSize, cellW);

        // Keep tiles square to avoid tall stretched rectangles.
        var cellH = cellW;

        // Default.css sets width/height with !important, so we must override with inline !important.
        $(".matrix").each(function () {
            this.style.setProperty('width', cellW + 'px', 'important');
            this.style.setProperty('height', cellH + 'px', 'important');
        });
...
function buildPage() {
    ...
    $(".resizeable").resizable(
        {
            ...
            stop: function (e, ui) {
                ...
                // `newTab` is not in scope here; resize the currently active tab.
                resizeMapAndMatrix(currentTab);
            }
        });
    ...
    $(".portlet-toggle").click(function () {
        ...
        icon.closest(".portlet").find(".portlet-content").slideToggle(150, function () {
            // After expand/collapse, ensure the map/grid area is in a consistent state and reflow.
            // Using a small timeout avoids sizing based on a not-yet-laid-out element.
            setTimeout(function () {
    ...
    if (currentTab.indexOf("Overview") > -1) {
        ...
        // Default to Map on first load so users always see the map immediately.
        // (Previously this could be "Grid" from stored settings, which hides the map.)
        $(".mapGrid").val("Map");
        $('#headerStrip').show();
